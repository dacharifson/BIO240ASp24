...
6) Make an appropriate graph and/or table for gender and describe its distribution.  Is there equal representation of gender in this sample of consumers?
...
11) The following code will let you examine the data for the two largest ethnic groups, Hispanic and White non-Hispanic, which comprise the majority of the data. Here, we will subset data using brackets that specify specific criteria of interest. Subsetting is important for portioning out the data based on some criteria. 

It has the form of: dataset$variable[dataset$variable=="criteria for subset"] 
And remember that capitalization matters
...
Also, there is a function that can also subset data that we will not be using today. The function is subset(). With the form: subset(dataset, criteria as an expression) or subset(dataset, variable=="category") for categorical subsetting.
...
Note: the code in the chunk below saves the subsetted values as their own objects (dds.hispanics and dds.white.non.hisp) in the environment. This allows you to call them in a more succinct way. 

```{r}
#Note: the "," at the end of the brackets is not a typo.
#Also Note: you can comment coding regions using "#"! 
#Comments using "#" are treated as text and not actual code.
